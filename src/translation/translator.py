    ...
    def try_all_keys_for_translation(self, text: str, model_name: str, conversation=None, max_attempts=20) -> Tuple[str, Any]:
        ...
        while total_attempts < max_attempts and len(keys_tried) < len(self.all_keys):
            ...
            try:
                ...
                prompt = f"""
              خودت را به عنوان یک مهندس نرم افزار در نظر بگیر که میخواهد یک متن انگلیسی را به فارسی ترجمه کند . باید اصطلاحات و کلمات فنی مهندسی نرم افزار را به انگلیسی نگهدارد و بقیه متن را به فارسی ترجمه کند و برای این کار باید از دستورالعمال های زیر پیروی کنی : 
            1. به عنوان مثال یک سری از مواردی که نباید ترجمه کنید را در زیر میگم و آن را یاد بگیرید برای ترجمه بهتر:
            ...
            2. اگر بخش‌هایی از متن حاوی کد برنامه‌نویسی است، آن را داخل تگ‌های <pre><code class="language-زبان_برنامه_نویسی"> و </code></pre> قرار دهید.
               برای مثال، کد جاوا را به این صورت قرار دهید: <pre><code class="language-java">کد جاوا</code></pre>
            
            3. بسیار مهم: ساختار متن اصلی را حفظ کنید. پاراگراف‌بندی، سرفصل‌ها، لیست‌ها و بخش‌های کد را با تگ‌های HTML مناسب حفظ کنید:
            ...
            4. متن شما باید یک متن html با شد که زیبا سازی شده باشد و فضا های اضافه برای زیبا سازی ایجاد شده باشد و یک متن تمیز و زیبا با استفاده از تگ های html ساخته شده باشد 
                
            5.شما نباید یک قالب کامل html بسازی و برگردانی برای من . بلکه باید یک div رگردانی که تمام مواردی که من از تو خواسته ام به شکل تگ های html باشد که من از ان ها در صفحات html خودم استفاده کنم

            6.از تگ های بیشتری برای زیبا سازی خروجی ترجمه شده استفاده کن مانند bold کردن italic کردن یا سر خط رفتن یا فاصله گذاشتن یا عنوان ها را درشت کردن تمام این موارد را بر اساس مفهوم متن اعمال کن

            این متن را به زبان فارسی ترجمه کن و همیشه در نظر داشته باش که کل متن باید به زبان فارسی ترجمه شود :
